    ...
    for idx, filename in enumerate(files, start=1):
        ...
        while True:
            ...
            if frame_count % 30 == 0:  # Adjust as needed
                try:
                    extract_faces_from_frame(frame, output_folder, filename[:-4], frame_count)
        ...
        cap.release()
        end_time = time.time()
        print(f"Done processing video {idx}/{len(files)}: {filename}. Time spent: {end_time - start_time:.2f} seconds")
